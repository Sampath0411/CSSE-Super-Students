...
export default function FacultyTimetablePage() {
  ...
  const handleRestoreClick = (entry: TimetableEntry) => {
    if (!faculty) return;
    restorePeriod(entry.day, entry.period, faculty.id, faculty.name);
    loadTimetableData();
  };

  const confirmCancel = () => {
    if (!selectedEntry || !faculty) return;
    cancelPeriod(selectedEntry.day, selectedEntry.period, faculty.id, faculty.name, reason);
  ...
  const confirmSubstitute = () => {
    if (!selectedEntry || !faculty || !substituteTeacherId) return;
    assignSubstitute(selectedEntry.day, selectedEntry.period, substituteTeacherId, faculty.id, faculty.name, reason);
  ...
  return (
    <div className="space-y-6">
      ...
      {modifications.length > 0 && (
        <div className="p-4 rounded-lg flex items-start gap-3 bg-amber-50 border border-amber-200">
